...
function restoreChildren(path, children) {
  ...
  children.forEach(child => {
     ...
     */
    if (child.children) {
      const restorePath = PATH.resolve(config.get().HOME_PATH, config.get().SOURCE_PATH, child.path);
      // Recursion
...
function restore() {
  const diroctorJSONFile = PATH.resolve(config.get().HOME_PATH, config.get().DIROCTOR_LOCK_FILE);
  ...
  const restorePath = PATH.resolve(config.get().HOME_PATH, config.get().SOURCE_PATH);
